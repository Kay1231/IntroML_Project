        ...
        for key, val in parameters.items():
            ...
            if target_key not in sd:
                ...
                if ".base_model.model." in key:
                    candidate_keys.append(key.replace(".base_model.model.", "."))
                if key.endswith(".weight"):
                    candidate_keys.append(key[: -len(".weight")] + ".base_layer.weight")
                    if ".base_model.model." in key:
                        candidate_keys.append(
                            key.replace(".base_model.model.", ".")[: -len(".weight")]
                            + ".base_layer.weight"
                        )
                for candidate_key in candidate_keys:
                    if candidate_key in sd:
                        target_key = candidate_key
                        break
            try:
                if (concat_across_output):
                    sd[target_key].add_(val.cpu() * scaling_coeffs)
                else:
                    sd[target_key].add_(val.T.cpu() * scaling_coeffs)
            except Exception as exc:
                base_shape = tuple(sd[target_key].shape) if target_key in sd else None
                update_shape = tuple(val.shape)
                raise RuntimeError(
                    f"Failed to add merged task parameter for key={key!r}; target_key={target_key!r}; "
                    f"base_shape={base_shape}; update_shape={update_shape}; "
                    f"concat_across_output={concat_across_output}"
                ) from exc
        return base_model
